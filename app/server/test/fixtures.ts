...
async function one(db: Querier, sql: string, p: unknown[]) {
...
export async function seedRealShape(db: Querier): Promise<void> {
  ...
  const ensureTag = async (n: string) =>
    tagId.get(n) ??
    (tagId.set(n, (await one(db, "INSERT INTO tags(name) VALUES ($1) RETURNING id", [n])).id),
    tagId.get(n)!);
  ...
  for (const src of sources) {
    const sid = (
      await one(db, "INSERT INTO sources(name, base_url) VALUES ($1,$2) RETURNING id", [
        src.name,
        src.base,
      ])
    ).id;
      ...
      for (let k = 0; k < 4; k++) {
        ...
        for (let di = 0; di < DAYS.length; di++) {
          await db.query(
            ...
            [gid, crawlIds[di], DAYS[di], rating, baseVotes + di * (10 + (i % 20)), genre],
          ); // votes rise each day
        }
      }
    }
