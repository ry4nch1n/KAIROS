...
export async function loadGames(
  ...
  for (const r of games) {
    const game = await one(
      ...
      `INSERT INTO games(source_id, source_game_id, url, title, thumbnail_url, developer, description, engine, orientation, mobile, last_seen_at)
       ...
       ON CONFLICT (source_id, source_game_id) DO UPDATE SET
         ...
         is_live = true
       RETURNING id`,
      [sourceId, r.sourceGameId, r.url, r.title, r.thumbnailUrl, r.developer, r.description, r.engine, r.orientation, r.mobile, crawlDateISO]
    );
    ...
    const before = await db.query(
      ...
      [gameId, crawlId, crawlDateISO, r.rating, r.votes, r.featured, r.genre]
    );
