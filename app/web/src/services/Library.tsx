    ...
    <article className="bcard pcard">
      ...
      <div className="pcard-head">
        ...
        <div className="pcard-headmain">
          <div className="btags">
            {p.badge && (
              <span className={"ptag badge-" + p.badge}>{BADGE_LABEL[p.badge] || p.badge}</span>
            )}
            {p.loopFamily && (
              <span className="ptag lf">{LOOP_LABEL[p.loopFamily] || p.loopFamily}</span>
            )}
            <span className={"ptag st st-" + p.status}>{p.status}</span>
            {p.provenance && (
              <span className={"ptag prov-" + p.provenance}>
                {PROV_LABEL[p.provenance] || p.provenance}
              </span>
            )}
            {isUrl(p.source) && (
              <a
                className="prov-receipt"
                href={p.source}
                target="_blank"
                rel="noreferrer"
                title="The market evidence this pitch is grounded in"
              >
                receipt ↗
              </a>
            )}
      ...
      {(p.grayBoxDays != null || p.contentScope) && (
        <div className="pscope">
          {p.grayBoxDays != null && (
            <span
              className="pscope-chip clock"
              title="Estimated days to a testable gray-box loop — the Aug kill-gate clock. A loop only provable at month six is out of scope no matter the market."
            >
              ⏱ ~{p.grayBoxDays}d to a testable loop
            </span>
          )}
          {p.contentScope && (
            <span className="pscope-chip" title="Content bill vs. what this genre's buyers expect">
      ...
      <div className={"pfields" + (open ? " open" : "")} ref={fieldsRef}>
        ...
        {open &&
          detail.map(([label, val]) =>
            val ? (
              <div key={label} className="pfield-full">
                <span className="plabel">{label}</span>
                {val}
              </div>
            ) : null,
      ...
      {(p.browserFit !== null ||
        ...
        <div className="pscores">
          <span
            className="pfit-pair"
            title="Co-equal platform-fit axes — together they map which strategy route a pitch fits"
          >
            <span>
              Browser fit{" "}
              {p.browserFit !== null ? <Dots n={p.browserFit} /> : <em className="pna">n/a</em>}
            </span>
            <span>
              Steam fit{" "}
              {p.steamFit !== null ? <Dots n={p.steamFit} /> : <em className="pna">n/a</em>}
            </span>
          </span>
          <span>
            Build ease <Dots n={p.buildEase} />
          </span>
          {(p.marketability !== null || p.founderFit !== null) && (
            <span
              className="pfit-pair pjudge"
              title="The two lenses the commercial scores miss — first-session hook pull, and whether you'd still care in month four"
            >
              <span>
                Hook{" "}
                {p.marketability !== null ? (
                  <Dots n={p.marketability} />
                ) : (
                  <em className="pna">n/a</em>
                )}
              </span>
              <span>
                Founder fit{" "}
                {p.founderFit !== null ? <Dots n={p.founderFit} /> : <em className="pna">n/a</em>}
              </span>
            </span>
          )}
        </div>
...
const COLLECTION_BLURB: Record<string, string> = {
  prototype:
    "Playable builds and paper tests will collect here — each one linked back to the loop family it validates.",
};
...
function scoreMean(p: Pitch): number {
  ...
  // counted against it.
  const vals = [p.browserFit, p.steamFit, p.buildEase, p.marketability, p.founderFit].filter(
...
function evidenceChips(p: Pitch): { label: string; missing: boolean }[] {
  ...
  chips.push(
    p.evidence
      ? { label: "evidence noted", missing: false }
      : { label: "no evidence", missing: true },
  );
  chips.push(
    p.source ? { label: "source linked", missing: false } : { label: "no source", missing: true },
  );
  return chips;
}
    ...
    <div className="card">
      ...
      <table className="dtable">
        <thead>
          <tr>
            <th>Loop family</th>
            <th className="r" title="Non-shelved pitches — the live bets">
              Live
            </th>
            <th className="r" title="All pitches ever tagged to this family">
              Total
            </th>
            <th>Status</th>
            <th>Pitches</th>
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.family} className={r.active === 0 ? "lf-empty" : ""}>
              <td className="gname">
                {LOOP_LABEL[r.family] || r.family}
                <span className="minibar">
                  <i style={{ width: (r.active / maxActive) * 100 + "%" }} />
                </span>
              </td>
              <td className="r">
                {r.active || <span title="no live pitch yet — an opening">0</span>}
              </td>
              <td className="r">{r.total}</td>
              <td>
                {Object.keys(r.byStatus).length ? (
                  Object.entries(r.byStatus).map(([st, n]) => (
                    <span key={st} className={"ptag st st-" + st}>
                      {st} {n}
                    </span>
                  ))
                ) : (
...
export function Library({ hidden }: { hidden: boolean }) {
  ...
  const activeName = isLeaderboard
    ? "Leaderboard"
    : isFamilies
      ? "Loop-family coverage"
      : COLLECTIONS.find((c) => c.key === active)?.name || "";
  const shownPitches = active === "pitch" ? pitches : [];
  const shownItems =
    active === "pitch" || isLeaderboard || isFamilies ? [] : items.filter((i) => i.kind === active);
  const isEmpty =
      ...
      >
        ...
        {COLLECTIONS.map((c) => (
          ...
          >
            ...
            <span
              className="badge"
              style={{ background: counts[c.key] ? "var(--accent-soft)" : "var(--text-3)" }}
            >
              {counts[c.key] || 0}
            </span>
          </a>
        ...
        >
          ...
          <span
            className="badge"
            style={{ background: activePitchCount ? "var(--accent-soft)" : "var(--text-3)" }}
          >
            {activePitchCount}
          </span>
        </a>
      ...
      <main className="main">
        <div className="topbar">
          ...
          <h2>
            ...
            <small>
              {isLeaderboard
                ? "which candidate is winning — and what evidence would change that"
                : isFamilies
        ...
        <div className="content">
          {!loaded ? (
            <div className="bcard-grid">
              {[0, 1, 2].map((i) => (
                <div className="bcard skeleton" key={i} style={{ height: 220 }} />
              ))}
            </div>
          ) : isLeaderboard ? (
